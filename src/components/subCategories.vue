...
export default {
  ...
  watch: {
    promises() {
      if (this.promises === 0) {
        const resultList = this.getData([`subContent-${this.currentId}_category-list`]);
        this.$set(this.subCategories, 'content', resultList)
      }
    }
  },
  ...
  mounted() {
    ...
    const thisPageCategoriesRequest = {
      method: 'GET',
      requestURL: `/v1/categories?parent_id=${this.currentId}`,
      headers: this.getGlobals.headers,
      body: null,
      stateTarget: [`subContent-${this.currentId}_category-list`]
    }
          ...
          if (Object.keys(thisPageCategories).length === 0) {
            ...
            for (let key in thisPageCategories) {
              ...
              const getSubCategoryInnerCategoriesRequest = {
                ...
                headers: this.getGlobals.headers,
                stateTarget: [`subContent-${this.currentId}_category-list`, thisPageCategories[key].id, 'innerContent', 'innerCategories'],
              };
              ...
              const getSubCategoryInnerClubsRequest = {
                ...
                headers:  this.getGlobals.headers,
                body: null,
                stateTarget: [`subContent-${this.currentId}_category-list`, thisPageCategories[key].id, 'innerContent', 'innerClubs']
              }
              ...
              this.sendRequest(getSubCategoryInnerCategoriesRequest)
                  .then(() => {
                    ...
                    thisPageCategories[key].innerContent.innerCategories = this.getData(
                        [
                          `subContent-${this.currentId}_category-list`,
                    ...
                    this.sendRequest(getSubCategoryInnerClubsRequest)
                        .then(() => {
                          ...
                          for (let item in thisCategories) {
                            thisCategories[item].type = 'category'
                          }

                          const thisClubs = thisPageCategories[key].innerContent.innerClubs;

                          for (let item in thisClubs) {
                            thisClubs[item].type = 'club'
                          }
